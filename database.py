...
def admin_stats() -> dict:
    ...
    total_users = _execute("SELECT COUNT(*) AS c FROM users", fetch="one")
    total_runs  = _execute("SELECT COUNT(*) AS c FROM runs WHERE status='done'", fetch="one")
    total_pending = _execute("SELECT COUNT(*) AS c FROM recharges WHERE status='pending'", fetch="one")
    active_today  = _execute(
        "SELECT COUNT(*) AS c FROM users WHERE last_seen >= NOW() - INTERVAL '1 day'"
    ...
    return {
        ...
        "total_runs":    total_runs["c"]    if total_runs    else 0,
        "total_pending": total_pending["c"] if total_pending else 0,
