...
async def safe_edit(msg: Message, text: str, reply_markup=None, parse_mode=ParseMode.MARKDOWN):
    ...
    try:
        ...
        if "not modified" not in err:
            ...
            # Fallback: try plain text if markdown parse fails
            try:
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if granted:
        greeting = f"🎁 Hey *{user.first_name}*! You got *{FREE_CREDITS} free credits* to start!"
    else:
...
async def _run_and_notify(app, chat_id: int, json_text: str,
    ...
    success_msg = (
        ...
        f"──────────────────────\n"
        f"🪙 Credits left: *{final_credits}*"
    )

    await safe_edit(live_msg, success_msg, reply_markup=run_again_kb(), parse_mode=ParseMode.MARKDOWN)
...
async def cmd_stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🚀 Total Runs Done: *{s['total_runs']}*\n"
        f"⏳ Pending Recharges: *{s['total_pending']}*",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def post_init(app: Application):
    ...
    # Commands visible to normal users
    ...
    await app.bot.set_my_commands([
        ...
        BotCommand("pending",    "⏳ List pending recharges"),
        BotCommand("approve",    "✅ Approve a recharge"),
        BotCommand("reject",     "❌ Reject a recharge"),
        BotCommand("addcredits", "🪙 Add credits to user"),
